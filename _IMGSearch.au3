...
Func _IMGSearch_Area($findImage, $x1, $y1, $x2 = @DesktopWidth, $y2 = @DesktopHeight, $tolerance = 0, $center = True)
	...
	For $i = 0 To (UBound($findIMG) - 1) Step +1
		...
		Local $result_img = DllCall("ImageSearchDLL.dll", "str", "ImageSearch", "int", $x1, "int", $y1, "int", $x2, "int", $y2, "str", $findIMGX)

		If Not IsArray($result_img) Then ; dll not found or other error
			$return[0] = -1
			ExitLoop
		EndIf
		If $result_img[0] = "0" Then ; search not found
			$return[0] = 0
			ContinueLoop
		Else
			$return[0] = 1
			Local $array = StringSplit($result_img[0], "|")
			If $center Then
